function citypicker(){
	...
	for(var e = 0;e<cityindex.length;e++){
		...
		for(var i = 0;i<LAreaData.length;i++){
			...
			if(LAreaData[i].enName.substring(0, 1)==cityindex[e]){
				var $creatcity = "<p style='font-size: 15px' id='"+LAreaData[i].id+"' attr='"+LAreaData[i].platformName+"' class = 'cityper'"+ "name = '" +LAreaData[i].enName.toLowerCase()+"'" +" "+"text='"+ LAreaData[i].platformName+"'"+">" + LAreaData[i].platformName + "</p>";
				$(".cityindex").eq(e).show();
