    ...
    success: function (data) {
      ...
      if (data.list != null && data.list.length > 0) {
        for (var i = 0; i < data.list.length; i++) {
          ...
          var periodId = data.list[i].id;

          var periodName = data.list[i].name;

          //拼接
          var option = $("<option>").val(periodId).text(periodName);
  ...
  if(imgLength === 4){
    ...
    $.ajax({
      ...
      success: function (res) {
        console.log("/upload/uploadMobileFile", res);
        //返回后台的图像数组名称
        //原来的
        var value = $('#imgs').val();
        //多张再拼接
        ...
        success: function (res) {
          ...
          $.ajax({
            ...
            success: function (res) {
              ...
              var mediaName ="chr2"+ (res.fileName).substr(0,res.fileName.lastIndexOf("."));
