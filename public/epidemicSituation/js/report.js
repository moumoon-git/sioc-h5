...
function upFileImg (files) {
  ...
  $.ajax({
    async: false,
    type: "POST",
    url: SERSVER_BASE_URL + "/upload/uploadMobileMsg",
    data : formData,
    cache: false,
    processData: false,
    contentType: false,
...
function removeAudioOperation(index){
    $("#"+index).remove();
    //取出选中删除的图片
    var audioSelectedValue = index.replace("mly",'') + ".mp3";
    //取出图片集合
    ...
    success: function (res) {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          //路径
          var filePath = SERSVER_BASE_URL + "/wx/videos/" + data.fileName;

          var mediaName ="mly" + (data.fileName).substr(0,data.fileName.lastIndexOf("."));
  ...
  $.ajax({
    ...
    success: function (data) {
      if(data && data.code===0){
        alert("上传成功");
        $('#btn').attr('disabled', false);
        window.location.href = './propertyReport.html'
      }else {
        alert(data.msg)
      }
...
function removeImgOperation(index) {
    ...
    //取出选中删除的图片
    var imgSelectedValue = index.replace("es",'') + ".jpg";
    //取出图片集合
    ...
    if(imgsValue.indexOf(",") > -1){
        ...
        //检索
        var index = imgArray.indexOf(imgSelectedValue);
