<!--
 * @Author: yaoweixin
 * @Date: 2020-11-23 16:57:52
 * @LastEditors: yaoweixin
 * @LastEditTime: 2020-11-23 19:44:55
 * @Description: file content
 * @FilePath: \vue-cli-multiPage-master\src\pages\workordermanager\chooseContacterListNew.vue
-->
<template>
    <div class="example">
     ...
     <div class="departmentcontainer">
      ...
      <template v-if="peopleShowVisible">
        <div v-for="(item,index) in peopleShowList" :key="index" class="contactslide">
          <van-checkbox v-model="item.checked"  @click="addContacterList(item)"></van-checkbox>
          <div class="personavatar">{{item.name?item.name.substring(item.name.length-2):''}}</div>
                      <div class="personmessage">
                          <div class="personcontainer">
...
export default {
  name: 'chooseContacterListNew',
  data() {
    return {
      nodeIdList: [],//保存点进去的节点id信息
      userList: [],//弹框的用户数据
      personNum: 0,//选中人数
      ...
      peopleVisible:false,
      peopleList:[],
      peopleShowVisible: false,//控制是否显示列表下的人
      peopleShowList: [],
    }
  },
  components: {
  ...
  methods: {
    deletePerson(userId) {//删除弹框中的人
      let pIndex =  this.contacterList.findIndex((item) => {
      ...
      if(event.children){
        ...
        this.$http({
          ...
          if(data.data.list.length!=0){
            this.peopleShowVisible = true
            var that=this
            Promise.resolve(data.data.list.forEach(item=>{
              item.checked=false
            })).then(()=>{
              that.peopleShowList=data.data.list
              for(let i=0;i<that.contacterList.length;i++){
                for(let j=0;j<that.peopleShowList.length;j++){
                  if(that.contacterList[i].id==that.peopleShowList[j].id){
                
                    that.peopleShowList[j].checked=true
